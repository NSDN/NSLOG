...
namespace HID
{
    ...
    public class Hid
    {
        ...
        private static bool IS_64BIT;

        /// <summary>
        /// 构造函数
        /// </summary>
        public Hid(bool is64bit)
        {
            IS_64BIT = is64bit;
        }
            ...
            if (hidInfoSet != IntPtr.Zero)
            {
                ...
                for (index = 0; index < MAX_USB_DEVICES; index++)
                {
                    ...
                    if (SetupDiEnumDeviceInterfaces(hidInfoSet, IntPtr.Zero, ref hUSB, index, ref interfaceInfo))
                    {
                        ...
                        detail.cbSize = Marshal.SizeOf(typeof(SP_DEVICE_INTERFACE_DETAIL_DATA)) + (IS_64BIT ? 2 : 0);
                        Marshal.StructureToPtr(detail, pDetail, false);
                        if (SetupDiGetDeviceInterfaceDetail(hidInfoSet, ref interfaceInfo, pDetail, buffsize, ref buffsize, IntPtr.Zero))
                        {
                            deviceList.Add(Marshal.PtrToStringAuto((IntPtr)(pDetail.ToInt64() + 4)));
                        }
